...
# --- Configuration ---
TARGET_SITES = ["https://www.1tamilmv.cards/", "https://www.1tamilblasters.study/"]
...
def get_root_title(raw_title):
    ...
    base_name = (
        year_match.group(1) if year_match else re.split(r"[\[\-\(]", raw_title)[0]
    )
...
if not os.path.exists(_file("history_urls.txt")):
    logger.info(
        "[*] First run detected. Creating a baseline index of BOTH sites to prevent bot flooding..."
    )
    with open(_file("history_urls.txt"), "w") as f:
        for site in TARGET_SITES:
            ...
            if html:
                ...
                topics = set(
                    re.findall(
                        r'https://www\.1tamil(?:mv|blasters)\.[a-z]+/index\.php\?/forums/topic/[^\s"\'><]+',
                        html,
                    )
                )
                for t in topics:
                    f.write(t + "\n")
    logger.info(
        "✅ Baseline established! Bot will only parse topics posted from this moment forward."
    )

if not os.path.exists(_file("feed.xml")):
    logger.info("Creating initial empty feed.xml")
    build_rss([])

# Load active state databases
history_urls = (
    set(open(_file("history_urls.txt")).read().splitlines())
    if os.path.exists(_file("history_urls.txt"))
    else set()
)
history_titles = (
    set(open(_file("history_titles.txt")).read().splitlines())
    if os.path.exists(_file("history_titles.txt"))
    else set()
)
magnets_db = (
    json.load(open(_file("db.json"))) if os.path.exists(_file("db.json")) else []
)
    ...
    try:
        for site_url in TARGET_SITES:
            ...
            if html:
                ...
                if new_topics:
                    logger.info(
                        f"[+] Found {len(new_topics)} new posts on {site_url}! Processing..."
                    )
                    ...
                    for topic in new_topics:
                        ...
                        if not page_html:
                            continue
                        raw_magnets = set(
                            re.findall(r'magnet:\?xt=urn:btih:[^\s"\'><]+', page_html)
                        )
                        ...
                        for mag in raw_magnets:
                            ...
                            title = re.sub(
                                r"www\.1Tamil(?:MV|Blasters)\.[a-z]+\s*-\s*",
                                "",
                                title,
                                flags=re.IGNORECASE,
                            )
                            ...
                            if any(
                                resolution in title_lower
                                for resolution in ["4k", "2160p", "uhd"]
                            ):
                                logger.info(f"⏩ Filtered Out (4K): {title}")
                                continue

                            size_match = re.search(
                                r"(\d+(?:\.\d+)?)\s*(gb|mb)", title_lower
                            )
                            if size_match:
                                if (
                                    size_match.group(2) == "gb"
                                    and float(size_match.group(1)) > 4.0
                                ):
                                    logger.info(
                                        f"⏩ Filtered Out (Oversized {size_match.group(1)}GB): {title}"
                                    )
                                    continue
                            ...
                            if (
                                root_title not in history_titles
                                and root_title != "unknown release"
                            ):
                                ...
                                logger.info(
                                    f"🛑 Ignored Duplicate Across Sites: {title}"
                                )
        ...
        logger.exception("An error occurred during the scanning loop")

    logger.info("💤 Standby mode active for 5 minutes...")
