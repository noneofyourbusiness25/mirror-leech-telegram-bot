...
def bypass_cloudflare(gatekeeper_url):
    ...
    if form:
        ...
        post_data_dict = {
            inp.get("name"): inp.get("value", "") for inp in inputs if inp.get("name")
        }
        ...
        if post_data_dict:
            ...
            try:
                post_resp = session.post(
                    action_url, data=post_data_dict, allow_redirects=True, timeout=20
                )
                ...
                if (
                    "googleusercontent.com" in resolved_url
                    or "drive.google" in resolved_url
                ):
                    return resolved_url
            except Exception as e:
            ...
            if href and "http" in href and "fast-dl" not in href:
                try:
                    if "session" in locals():
                        ...
                        for fa in BeautifulSoup(
                            final_resp.text, "html.parser"
                        ).find_all("a"):
                            if "googleusercontent.com" in fa.get("href", ""):
                                return fa.get("href")
...
if not os.path.exists(_file("vega_feed.xml")):
    logger.info("Creating initial empty vega_feed.xml")
    build_rss([])

# Load databases - ISOLATED FILE NAMES
history_titles = (
    set(open(_file("vega_history_titles.txt")).read().splitlines())
    if os.path.exists(_file("vega_history_titles.txt"))
    else set()
)
history_gatekeepers = (
    set(open(_file("vega_history_gatekeepers.txt")).read().splitlines())
    if os.path.exists(_file("vega_history_gatekeepers.txt"))
    else set()
)
movies_db = (
    json.load(open(_file("vega_db.json")))
    if os.path.exists(_file("vega_db.json"))
    else []
)

while True:
    logger.info("--- ⏰ Scanning Vegamovies Index ---")
    feed_updated = False
    ...
    try:
        ...
        if sol:
            ...
            if new_posts:
                logger.info(
                    f"[+] Found {len(new_posts)} new or updated posts! Processing..."
                )
                    ...
                    if target_hub_link:
                        ...
                        for gate_url in gatekeepers:
                            if gate_url not in history_gatekeepers:
                                ...
                                with open(
                                    _file("vega_history_gatekeepers.txt"), "a"
                                ) as f:
                                    f.write(gate_url + "\n")
